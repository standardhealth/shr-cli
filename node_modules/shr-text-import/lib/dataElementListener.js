...
class DataElementImporter extends SHRDataElementParserListener {
  ...
  enterField(ctx) {
    ...
    if (field instanceof IncompleteValue && field.identifier && field.identifier.isValueKeyWord) {
      if (this._currentDef.value) {
        //11024 , 'Elements cannot use ${value} modifier and specify  ${value} field at same time.' , 'Unknown', 'errorNumber'
        logger.error( {value:  this._currentDef.value },'11024' );
        return;
      }
    ...
    if (ctx.elementWithConstraint()) {
      ...
      if (ewc.simpleOrFQName() || ewc.specialWord()) {
        ...
        if (refIdentifier.isSpecialKeyWord) {
          //11026 , 'ref( ${value} ) is an unsupported construct; treating as ${value} without the reference.' , 'Unknown', 'errorNumber'
          logger.error({ value:  refIdentifier.name }, '11026' );
  ...
  resolveValueSetForVSConstraint(vsConstraint) {
    ...
    if (vsConstraint.valueset().PATH_URL()) {
      ...
      if (resolution.error) {
        //13079, 'Resolution error ${errorText} ' ,  'Unknown' , 'errorNumber'
        logger.error({errorText : resolution.error }, '13079');
      }
      if (resolution.url) {
